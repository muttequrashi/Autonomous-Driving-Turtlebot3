...
# Some part of code specially for edge_detection and lane is borrowed from Addison Sears-Collins and changed to meet the requirements
...
class Lane:
  ...
  def __init__(self, orig_frame):
    ...
    self.roi_points = np.float32([
      (int(0.353*width),int(0.775*height)), # Top-left corner
      (0.1375*width, height-1), # Bottom-left corner           
      (int(0.946*width),height-1), # Bottom-right corner
      (int(0.8156*width),int(0.775*height)) # Top-right corner
    ])
...
class lane_following:
    def __init__(self):
        self.image_lane_sub = rospy.Subscriber('/camera/image', CompressedImage, self.callback, queue_size = 1, buff_size=2**24)
    ...
    def callback(self, image_msg):
        ...
        msg_desired_center.data = 500

        #publish 
        self.pub_lane.publish(msg_desired_center)
        self.detect_mask_lane_pub.publish(self.cvBridge.cv2_to_compressed_imgmsg(warped_frame, "jpg"))
        self.detect_midlane_pub.publish(self.cvBridge.cv2_to_compressed_imgmsg(frame_with_lane_lines2, "jpg"))
